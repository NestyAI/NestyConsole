...
} from "@/lib/builtin-providers/server";
...
export async function PUT(request: Request, context: RouteContext) {
  ...
  return withRuntimeAdmin(async ({ credentials }) => {
    const result = await updateBuiltinProviderApiKey(providerId, body, credentials);
    if (!result.ok) {
...
export async function DELETE(_request: Request, context: RouteContext) {
  const { providerId } = await context.params;
  return withRuntimeAdmin(async ({ credentials }) => {
    const result = await deleteBuiltinProviderApiKey(providerId, credentials);
    if (!result.ok) {
      return gatewayResultToResponse(result);
    }
    return runtimeSuccessResponse(result.data, result.status);
