...
import type { ProviderHealthLatestResponse } from "@/lib/gateway/types";
...
export async function GET(request: Request) {
  ...
  const path = query.toString()
    ? `/internal/diagnostics/provider-health/latest?${query.toString()}`
    : "/internal/diagnostics/provider-health/latest";
  const result = await gatewayFetch<ProviderHealthLatestResponse>(
    path,
    {},
